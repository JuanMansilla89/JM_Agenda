---
project: Antamina
status: active
stage: propuesta-tecnica-en-elaboracion
owner: "Juan Mansilla"
stakeholders: []
start-date: 2026-06-02
tags: [antamina]
onedrive: ""
---

# Antamina

## Sub-proyectos

| Sub-proyecto | Estado | Etapa |
|-------------|--------|-------|
| [[GNSS_Palas/GNSS_Palas\|GNSS_Palas]] | 🟡 Activo | propuesta-tecnica-en-elaboracion |

## Estado actual

| Aspecto | Estado |
|---------|--------|
| GNSS_Palas — propuesta técnica | ⏳ En elaboración |

## Objetivo

Cliente Antamina (Compañía Minera Antamina S.A.), mina polimetálica a tajo abierto en Ancash, Perú. Este nodo agrupa los proyectos y servicios que ASTAY Systems desarrolla para Antamina.

## Tareas activas

## Tareas completadas

## Decisiones clave

| Fecha | Decisión | Justificación | Estado |
|-------|----------|---------------|--------|
| 2026-06-02 | Crear sub-proyecto GNSS_Palas | Primera iniciativa Antamina | #decision |

## Riesgos

| Riesgo | Probabilidad | Impacto | Mitigación |
|--------|-------------|---------|------------|

## 📎 Documentación

```dataview
TABLE file.mtime AS "Modificado", tipo AS "Tipo", fuente AS "Fuente"
FROM "30-Projects/Antamina/Documentacion"
WHERE file.name != ".gitkeep"
SORT file.mtime DESC
```

| Tipo | Carpeta | Template |
|------|---------|----------|
| 📋 Reunión | `Documentacion/Reuniones/` | `Doc-Reunion` |
| 📧 Email / mensaje | `Documentacion/Comunicaciones/` | `Doc-Comunicacion` |
| 🌐 Referencia | `Documentacion/Referencias/` | `Doc-Referencia` |
| 📄 Entregable | `Documentacion/Entregables/` | `Quarto-Word.qmd` |

## ☁️ OneDrive

[📁 Carpeta Antamina](PENDIENTE — pegar URL de OneDrive)

## Recursos y referencias

- 

---
*Última actualización: 2026-06-02*
